...
app.post('/api/machines', async (req, res) => {
  const { hostname, model_info, usage_desc, memo, last_alive, cpu_info, cpu_arch, memory_size, disk_info, os_name, interfaces } = req.body;
  ...
  try {
    ...
    await conn.query(
      'INSERT INTO machines (id, hostname, model_info, usage_desc, memo, last_alive, cpu_info, cpu_arch, memory_size, disk_info, os_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [machineId, hostname, model_info, usage_desc, memo, last_alive, cpu_info || '', cpu_arch || '', memory_size || '', disk_info || '', os_name || '']
    );
...
app.put('/api/machines/:id', async (req, res) => {
    ...
    try {
      ...
      await conn.query(
        'UPDATE machines SET hostname=?, model_info=?, usage_desc=?, memo=?, last_alive=?, cpu_info=?, cpu_arch=?, memory_size=?, disk_info=?, os_name=? WHERE id=?',
        [hostname, model_info, usage_desc, memo, last_alive, cpu_info || '', cpu_arch || '', memory_size || '', disk_info || '', os_name || '', machineId]
      );
...
app.get('/api/machines/search', async (req, res) => {
  ...
  try {
    ...
    const [machines] = await db.query(
      'SELECT * FROM machines WHERE hostname LIKE ? OR model_info LIKE ? OR usage_desc LIKE ? OR memo LIKE ? OR cpu_info LIKE ? OR cpu_arch LIKE ? OR memory_size LIKE ? OR disk_info LIKE ? OR os_name LIKE ?',
